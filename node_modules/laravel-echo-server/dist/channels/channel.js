...
var Channel = (function () {
    ...
    Channel.prototype.join = function (socket, data) {
        ...
        if (data.channel) {
            if (this.isPrivate(data.channel)) {
                ...
                var index = _channel.indexOf('.');
                axios_1.default.post(this.options.authHost + "/api/joinExposureChannel", {
                    broker_id: _channel.substring(++index),
                    socket_id: socket.id
                }).then(function () {
                    if (_this.options.devMode) {
                        log_1.Log.info("[" + new Date().toISOString() + "] - call joinExposureChannel successfully~");
                    }
                }).catch(function (error) {
                    if (_this.options.devMode) {
                        log_1.Log.error("[" + new Date().toISOString() + "] - call joinExposureChannel fail, with error: " + error);
                    }
                });
            }
            else {
    ...
    Channel.prototype.leave = function (socket, channel, reason) {
        ...
        if (channel) {
            ...
            if (this.options.devMode) {
                log_1.Log.info("[" + new Date().toISOString() + "] - " + socket.id + " left channel: " + channel.indexOf('.') + " (" + reason + ")");
            }
            var _channel = channel;
            var index = _channel.indexOf('.');
            axios_1.default.post(this.options.authHost + "/api/stopExposurePush", {
                broker_id: _channel.substring(++index)
            }).then(function () {
                if (_this.options.devMode) {
                    log_1.Log.info("[" + new Date().toISOString() + "] - call stopExposurePush successfully~");
                }
            }).catch(function (error) {
                if (_this.options.devMode) {
                    log_1.Log.error("[" + new Date().toISOString() + "] - call stopExposurePush fail, with error: " + error);
                }
            });
        }
    };
